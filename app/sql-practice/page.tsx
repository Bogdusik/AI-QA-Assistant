...
function SqlBlock({ sql }: { sql: string }) {
  return (
    <pre className="max-w-full overflow-auto rounded-lg border border-white/10 bg-white/5 p-4 text-xs text-white/70">
      {sql}
    </pre>
  );
}
      ...
      <Section title="Quick SQL reminders (what to practice)">
        <div className="grid gap-4 md:grid-cols-2">
          <Card className="space-y-2">
            ...
            <p className="text-sm text-white/60">Choose which columns you want to see.</p>
          </Card>
          <Card className="space-y-2">
            <p className="font-semibold text-white">WHERE</p>
            <p className="text-sm text-white/60">Filter rows (e.g., type, status, userId).</p>
          </Card>
          <Card className="space-y-2">
            <p className="font-semibold text-white">ORDER BY</p>
            <p className="text-sm text-white/60">Sort output (e.g., newest documents).</p>
          </Card>
          <Card className="space-y-2">
            <p className="font-semibold text-white">COUNT</p>
            <p className="text-sm text-white/60">Count matching records (e.g., pending items).</p>
          </Card>
        </div>
      </Section>
        ...
        <div className="space-y-4">
          ...
          <Card className="space-y-2">
            ...
            <SqlBlock
              sql={`SELECT "id", "title", "type", "createdAt"\nFROM "Document"\nORDER BY "createdAt" DESC\nLIMIT 5;`}
            />
            <p className="text-sm text-white/60">Try: add title search with LIKE.</p>
          </Card>
