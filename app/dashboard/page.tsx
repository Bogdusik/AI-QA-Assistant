...
export default async function DashboardPage() {
  ...
  const byType = {
    TEST_CASE_SET: docs.filter((d) => d.type === "TEST_CASE_SET").length,
    CHECKLIST: docs.filter((d) => d.type === "CHECKLIST").length,
    BUG_REPORT: docs.filter((d) => d.type === "BUG_REPORT").length,
    API_TEST_SET: docs.filter((d) => d.type === "API_TEST_SET").length
  };
  const allItems = docs.flatMap((d) => d.generatedItems);
  const byReview = {
    PENDING: allItems.filter((i) => i.reviewStatus === "PENDING").length,
    ACCEPTED: allItems.filter((i) => i.reviewStatus === "ACCEPTED").length,
    REJECTED: allItems.filter((i) => i.reviewStatus === "REJECTED").length
  };
